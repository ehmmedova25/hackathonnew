...
function chapterPage(index: number): number {
  return 6 + index * 16;
}

// Fallback data shown when localStorage is empty
const FALLBACK_BOOK: Book = {
  title: "Leyla Məmmədovanın hekayəsi",
  chapters: [
    {
      title: "Uşaqlıq illəri",
      summary: "",
      quote: "Mən düşünmürdüm ki, həyatım bir gün kitab olacaq.",
      image_prompt: "",
      image_url: "",
    },
    { title: "Müharibə xatirələri", summary: "", quote: "", image_prompt: "", image_url: "" },
    { title: "Sevgi və ailə",       summary: "", quote: "", image_prompt: "", image_url: "" },
    { title: "Həyat dərsləri",      summary: "", quote: "", image_prompt: "", image_url: "" },
  ],
};
...
function PreviewPage() {
  ...
  const [book, setBook] = useState<Book>(FALLBACK_BOOK);

  useEffect(() => {
    const url = localStorage.getItem("memoir_pdf_url");
    const bookRaw = localStorage.getItem("memoir_book");

    if (url) setPdfUrl(url);

    if (bookRaw) {
      try {
        const parsed: Book = JSON.parse(bookRaw);
        if (parsed?.title && Array.isArray(parsed?.chapters)) {
          setBook(parsed);
        }
      } catch (e) {
        console.error("memoir_book parse xətası:", e);
      }
    }
  }, []);
  ...
  // First chapter with a non-empty quote
  const quoteChapter = book.chapters.find((c) => c.quote?.trim()) ?? book.chapters[0];
  // Image for the spread preview — first chapter image or fallback
  const spreadImage = book.chapters[0]?.image_url || hero4;
  // Cover illustration — first chapter image or local asset
  const coverImage = book.chapters[0]?.image_url || illus;
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                  ...
                  disabled={!pdfUrl}
                  className="inline-flex items-center gap-2 rounded-full bg-foreground text-background px-5 py-2.5 text-xs uppercase tracking-[0.18em] hover:bg-brown transition disabled:opacity-50"
              >
          ...
          <div className="max-w-7xl mx-auto grid lg:grid-cols-[1.1fr_0.9fr] gap-14 lg:gap-24 items-start mt-12 md:mt-16">
            ...
            <div className="relative">
                ...
                >
                  ...
                  <div className="absolute inset-x-10 top-20 h-[52%] rounded-md overflow-hidden">
                    <img
                        ...
                        loading="lazy"
                        onError={(e) => {
                          (e.currentTarget as HTMLImageElement).src = illus;
                        }}
                    />
                  ...
                  <div className="absolute inset-0 p-10 flex flex-col">
                    ...
                    <div className="mt-auto">
                      ...
                      <div className="flex items-center justify-between text-[10px] uppercase tracking-[0.3em] text-foreground/55 mt-4">
                        ...
                        <span>
                        {toRoman(1)} — {toRoman(book.chapters.length)}
                      </span>
                      </div>
                    </div>
                  </div>
                ...
                >
                  ...
                  <div className="relative bg-paper p-6 aspect-[3/4]">
                    <div className="flex items-baseline justify-between text-[9px] uppercase tracking-[0.3em] text-brown">
                      <span>Fəsil {toRoman(1)}</span>
                      <span className="text-foreground/40">{chapterPage(0)}</span>
                    </div>
                    <h3 className="serif text-2xl mt-3 leading-tight">
                      {book.chapters[0]?.title}
                    </h3>
                    <div className="mt-3 w-10 h-px bg-brown/40" />
                    <p className="serif italic text-[13px] text-foreground/75 mt-4 leading-relaxed line-clamp-5">
                      "{book.chapters[0]?.quote || book.chapters[0]?.summary || "..."}"
                    </p>
                  ...
                  <div className="relative bg-beige p-4 aspect-[3/4] flex flex-col">
                    <div className="relative flex-1 rounded-sm overflow-hidden">
                      <img
                          ...
                          loading="lazy"
                          onError={(e) => {
                            (e.currentTarget as HTMLImageElement).src = hero4;
                          }}
                      />
              ...
              <span className="w-6 h-6 rounded-full bg-brown/10 text-brown flex items-center justify-center">
                ...
                <span className="serif italic text-foreground/80">
                {book.chapters.length} illustrasiya yaradıldı
              </span>
              </motion.div>
            ...
            >
              ...
              <div className="mt-8 grid grid-cols-3 gap-px bg-cream rounded-2xl overflow-hidden border border-cream">
                {[
                  { k: "Səhifə", v: String(book.chapters.length * 16 + 8) },
                  { k: "Fəsil",  v: String(book.chapters.length) },
                  { k: "İllustrasiya", v: String(book.chapters.filter((c) => c.image_url).length || book.chapters.length) },
                ].map((m) => (
              ...
              <div className="mt-10">
                ...
                <ul className="mt-5 divide-y divide-cream border-y border-cream">
                  {book.chapters.map((c, i) => (
                      ...
                      >
                        <span className="serif italic text-brown w-6">{toRoman(i + 1)}</span>
              ...
              <div className="mt-12 flex flex-wrap items-center gap-3">
                <button
                    onClick={handleDownload}
                    disabled={!pdfUrl}
                    className="group inline-flex items-center gap-3 rounded-full bg-foreground text-background px-8 py-4 text-sm uppercase tracking-[0.18em] hover:bg-brown transition-colors duration-500 shadow-xl shadow-brown/20 disabled:opacity-50"
                >
                  <Download className="w-4 h-4" /> PDF yüklə
                </button>
                <Link
              ...
              <span className="inline-flex items-center gap-2">
                ...
                {book.chapters.length * 16 + 8} səhifə · {book.chapters.length} fəsil
              </span>
                <span className="inline-flex items-center gap-2">
                <FileText className="w-3.5 h-3.5" /> PDF · çap üçün hazır
              </span>
              </div>
            </motion.div>
          </div>
